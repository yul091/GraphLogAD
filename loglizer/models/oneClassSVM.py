...
class OneClassSVM(object):
    ...
    def evaluate(self, X, y_true):
        ...
        precision, recall, f1, auc, aupr = metrics(y_pred, y_true)
        print('Precision: {:.3f}%, recall: {:.3f}%, F1-measure: {:.3f}%, AUC: {:.3f}%, AUPR: {:.3f}%\n'.format(precision, recall, f1, auc, aupr))
        end = time.time()
        print('Total test time: {:.3f} s\n'.format(end - start))
